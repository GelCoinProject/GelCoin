 ...
 *
 *   // network
 *   CGelCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.gelcoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGelCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGelCoinNetwork(UniValue objIn)
    {
...
// // can be under: GelCoinNetwork
// //   -- signature requirements : Key1(User)
// class CGelCoinNetworkVariable : public CGovernanceObject
// {
// private:
