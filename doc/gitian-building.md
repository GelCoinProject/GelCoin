...
git clone https://github.com/gelcoinpay/gelcoin
```
...
To build GelCoin (for Linux, OS X and Windows) just follow the steps under 'perform
...
./bin/gbuild --commit gelcoin=${COMMIT} --url gelcoin=${URL} ../gelcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gelcoin=${COMMIT} --url gelcoin=${URL} ../gelcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gelcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gelcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gelcoin.git
